...
const { getSeminar } = useSeminar();

const seminars = computed(() => seminarStore.getSeminar);
        ...
        <Table v-if="seminars">
          <TableHeader>
            <TableRow>
              ...
              <TableHead
                v-if="
                  seminars.some((seminar) =>
                    ['rejected', 'cancelled'].includes(seminar.status)
                  )
                "
                >Actions</TableHead
              >
          ...
          <TableBody>
            <TableRow v-for="(row, index) in seminars" :key="row.id">
              ...
              <TableCell>
                <a
                  v-if="row.seminar_file"
                  :href="row.seminar_file"
                  download
              ...
              >
                <Button class="bg-yellow-500 text-white py-1 px-3 rounded-md">
                  <RouterLink :to="`/seminar/edit/${row.id}`">
                    Edit
                  </RouterLink>
                </Button>
              </TableCell>
