...
const { fetchSeminarById, updateSeminar } = useSeminar();

const seminarId = ref<string>(
  Array.isArray(route.params.id) ? route.params.id[0] : route.params.id
);

const loading = ref(false);
const seminarData = ref<Seminar | null | undefined>(undefined);
...
onMounted(async () => {
  ...
  const response = await fetchSeminarById(seminarId.value);

  if (response.success) {
    seminarData.value = response.data as Seminar;
    lecturerId.value = seminarData.value?.supervisor?.id ?? "";
    examinerId.value = seminarData.value?.examiner?.id ?? "";
    form.setValues({
      supervisor_id: lecturerId.value,
      examiner_id: examinerId.value,
      seminar_file: seminarData.value?.seminar_file ?? "",
    });
  }
  loading.value = false;
});
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  if (values.seminar_file instanceof File) {
    formData.append("seminar_file", values.seminar_file);
  } else if (typeof values.seminar_file === "string") {
    formData.append("seminar_file", values.seminar_file);
  }
  formData.append("_method", "PATCH");

  const result = await updateSeminar(seminarId.value, formData);
  if (result.success) {
  ...
  <NuxtLayout>
    <div class="p-6">
      <CardContent class="w-full bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold mb-6">Edit Seminar</h2>
        <form
          @submit="onSubmit"
        ...
        >
          ...
          <FormField v-slot="{ field }" name="seminar_file">
            <FormItem>
              <FormLabel>Upload File Seminar</FormLabel>
              <FormControl>
                <Input
                  type="file"
                  accept=".doc,.docx,.pdf"
              ...
              <p
                v-if="seminarData && seminarData.seminar_file"
                class="text-sm text-gray-500 mt-1"
              >
                📄 Existing file:
                <a
                  :href="seminarData.seminar_file"
                  target="_blank"
